...
void kill_virus(char *fileName, int signitureOffset, int signitureSize){
    ...
    for(int i= 0;i<signitureSize;i++){
        fwrite(&str,sizeof(char),1,infected); 
    }
    fclose(infected);
}
...
link* fixFile(link* c){
    ...
    int signatureOffset, signitureSize;
    printf("\nEnter file name\n");
    ...
    printf("\nEnter signiture offset\n");
    
    char t[100];
    fgets(t, 100, stdin);
    sscanf(t, "%d",&signatureOffset);
  
    printf("\nEnter signiture size\n");
    char m[100];
    fgets(m, 100, stdin);
    sscanf(m, "%d",&signitureSize);
    
    kill_virus(input_file, signatureOffset, signitureSize);
    fclose(f);
    return c;
...
struct fun_desc menu_display[] = {{"Load signatures", loadSig}, {"Print signatures", printSig}, {"Detect viruses",detect },{"Fix file",fixFile }, {"Quit", quit}, {NULL, NULL}};
